...
>
> 5）**Host**（发送请求时，该报头域是必需的） **作用: 请求报头域主要用于指定`被请求资源的Internet主机和端口号`，它通常从HTTP URL中提取出来的**
>
...
> 5. 服务器接收到客户端发来的密文之后，会**用自己的私钥对其进行非对称解密**，解密之后的明文就是客户端密钥，然后用客户端密钥，即对称密钥，对数据进行对称加密，这样数据就变成了密文。
